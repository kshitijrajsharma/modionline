...
<div>
    ...
    <form onsubmit="validateform()"  method="post" name="newsform" enctype="multipart/form-data">
        ...
        <div class="form-group">
          <label for="vehicle1"> Editor's Pick</label>
          <input type="checkbox" id="vehicle1" name="editor" value="yes">
  ...
  if(isset($_POST["submit"])){
    ...
    if($title!=""){
      ...
      move_uploaded_file($tmp_thumbnail,"images/$thumbnail");
            ...
            $query=mysqli_query($conn,"insert into news (title,description,date,category,Editor,thumbnail) values('$title','$description','$date','$category','$editor','$thumbnail')");
            if($query){
